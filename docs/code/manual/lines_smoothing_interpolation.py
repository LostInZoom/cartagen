...
sub1.set_title(f'Catmull-Rom smoothing', pad=10, family='sans-serif')
...
path = Path(numpy.asarray(c4.smooth_catmull_rom(line, 10).coords)[:, :2])
...
sub2.set_title(f'Chaikin smoothing', pad=10, family='sans-serif')
sub2.axes.get_xaxis().set_visible(False)
sub2.axes.get_yaxis().set_visible(False)

path = Path(numpy.asarray(line.coords)[:, :2])
sub2.add_patch(PathPatch(path, facecolor="none", edgecolor='gray', linewidth=1))
path = Path(numpy.asarray(c4.smooth_chaikin(line, 3).coords)[:, :2])
