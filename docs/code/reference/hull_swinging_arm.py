...
h1 = c4.hull_swinging_arm(points, 800)
h2 = c4.hull_swinging_arm(points, 2000)
h3 = c4.hull_swinging_arm(points, 3000)

fig = plt.figure(1, (12, 4))

sub1 = fig.add_subplot(131)
sub1.set_title("a) length=800", pad=10, family='sans-serif')
sub1.axes.get_xaxis().set_visible(False)
sub1.axes.get_yaxis().set_visible(False)

sub2 = fig.add_subplot(132)
sub2.set_title("b) length=2000", pad=10, family='sans-serif')
sub2.axes.get_xaxis().set_visible(False)
sub2.axes.get_yaxis().set_visible(False)

sub3 = fig.add_subplot(133)
sub3.set_title("c) length=3000", pad=10, family='sans-serif')
sub3.axes.get_xaxis().set_visible(False)
sub3.axes.get_yaxis().set_visible(False)

for p1 in h1:
    poly1 = Path.make_compound_path(Path(numpy.asarray(p1.exterior.coords)[:, :2]),*[Path(numpy.asarray(ring.coords)[:, :2]) for ring in p1.interiors])
    sub1.add_patch(PathPatch(poly1, facecolor="red", edgecolor='none'))

for p2 in h2:
    poly2 = Path.make_compound_path(Path(numpy.asarray(p2.exterior.coords)[:, :2]),*[Path(numpy.asarray(ring.coords)[:, :2]) for ring in p2.interiors])
    sub2.add_patch(PathPatch(poly2, facecolor="red", edgecolor='none'))

for p3 in h3:
    poly3 = Path.make_compound_path(Path(numpy.asarray(p3.exterior.coords)[:, :2]),*[Path(numpy.asarray(ring.coords)[:, :2]) for ring in p3.interiors])
