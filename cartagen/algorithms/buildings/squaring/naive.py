...
def square_polygon_naive(polygon, orientation='primary', angle_tolerance=8.0, correct_tolerance=0.6, remove_flat=True):
    """
    ...
    Notes
    -----
    By default, this algorithm is not very destructive, if you want to
    force edges onto an orthogonal coordinate system, you should use
    :func:`square_polygon_orthogonal <cartagen.square_polygon_orthogonal>`.
    ...
    # Calculate the orientation using the selected method
    o = polygon_orientation(polygon, orientation)
    vo = Vector2D.from_angle(o, 1)

    # exterior.coords includes the closing point (== first point), so we drop it.
    # nb_edges is therefore exactly the number of vertices / edges.
    points = list(polygon.exterior.coords)[:-1]
    nb_edges = len(points)  # FIX: was len(points) - 1, causing off-by-one errors
    ...
    def __get_vecs_around(i):
        v1 = (nb_edges - 1) if i == 0 else (i - 1)  # FIX: was hardcoded len(vecs) - 1
        v2 = i
        return v1, v2
    ...
    def __update():
        ...
        for i in range(0, nb_edges - 1):  # FIX: was len(vecs) - 1
            angles[i + 1] = np.pi - vecs[i].angle(vecs[i + 1])
            align[i] = abs(vecs[i].product(vo))
        align[nb_edges - 1] = abs(vecs[nb_edges - 1].product(vo))  # FIX: was len(align) - 1
    ...
    # Initial computation of angles and alignments
    angles[0] = np.pi - vecs[-1].angle(vecs[0])
    for i in range(0, nb_edges - 1):  # FIX: was len(vecs) - 1
        angles[i + 1] = np.pi - vecs[i].angle(vecs[i + 1])
        align[i] = abs(vecs[i].product(vo))
    ...
    # Pass 1: correct flat angles (close to 180°)
    ...
    # Pass 2: correct right and 45° angles
    for i in range(0, len(angles)):
        if abs(np.pi/2 - angles[i]) <= angle_tolerance and abs(np.pi/2 - angles[i]) > correct_tolerance:
            ...
            angle_before_isright = abs(angles[angle_before] - np.pi / 2) < correct_tolerance  # FIX: was comparing angle to itself
            angle_after_isright = abs(angles[angle_after] - np.pi / 2) < correct_tolerance    # FIX: idem
    ...
    # Pass 3: optionally remove flat vertices from the result
    result = []
    if remove_flat:
