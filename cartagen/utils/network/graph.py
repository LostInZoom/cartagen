...
def dissolve_network(network):
    ...
    This function is also suitable for river networks as it preserves the
    direction of the LineStrings. It also keeps crossing sections (tunnels and bridges).

    Parameters
    ----------
    network : GeoDataFrame of LineString
        The network to dissolve.
    ...
    for idx, row in network.iterrows():
        ...
        # Direction is defined from index 0 to index -1
        start, end = geom.coords[0], geom.coords[-1]
        attrs = row.to_dict()
        G.add_edge(start, end, key=idx, **attrs)

    # 2. Identify "Pass-through" nodes only
    # A node is safe to dissolve ONLY if it has 1 incoming and 1 outgoing edge
    nodes_to_simplify = [n for n in G.nodes() if G.in_degree(n) == 1 and G.out_degree(n) == 1]
    ...
    for node in nodes_to_simplify:
        ...
        in_edge = list(G.in_edges(node, data=True, keys=True))[0]
        out_edge = list(G.out_edges(node, data=True, keys=True))[0]
        ...
        # Merge geometries - Order matters! 
        # We put the incoming line first to preserve direction
        merged_geom = linemerge([d1['geometry'], d2['geometry']])
        
        if not isinstance(merged_geom, LineString):
            continue

        # Keep attributes from the longest segment
        new_attrs = d1.copy() if d1['tmp_length'] >= d2['tmp_length'] else d2.copy()
        new_attrs['geometry'] = merged_geom
        new_attrs['tmp_length'] = merged_geom.length

        # Remove old and add new directed edge
        G.remove_node(node)
        G.add_edge(u, v, **new_attrs)

    # 3. Convert back to GeoDataFrame
    simplified_edges = [data for _, _, data in G.edges(data=True)]
